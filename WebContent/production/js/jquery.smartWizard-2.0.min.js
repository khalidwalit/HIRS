 ...
 */
(function(a){
	a.fn.smartWizard=function(d){
		var c=a.extend({},a.fn.smartWizard.defaults,d);
		var b=arguments;
		return this.each(function(){
			var o=a(this);
			var n=c.selected;
			var x=a("ul > li > a[href^='#step-']",o);
			var t=0;
			var m,k,w,i,j,r,q;w=a(".actionBar",o);
			
			if(w.length==0){
				w=a("<div></div>").addClass("actionBar")
				}
				k=a(".msgBox",o);
				if(k.length==0){
					k=a('<div class="msgBox"><div class="content"></div><a href="#" class="">X</a></div>');
					w.append(k)}a(".close",k).click(function(){k.fadeOut("normal");return false});
					if(!d||d==="init"||typeof d==="object"){
						v()}else{if(d==="showMessage"){
							var s=Array.prototype.slice.call(b,1);
							l(s[0]);
							return true
							}else{
								if(d==="setError"){
									var s=Array.prototype.slice.call(b,1);
									e(s[0].stepnum,s[0].iserror);
									return true
									}else{
										a.error("Method "+d+" does not exist")
											...
											var A=o.children("div");
											o.children("ul").addClass("anchor");
											A.addClass("content");
											m=a("<div>Loading</div>").addClass("loader");
											w=a("<div></div>").addClass("actionBar");
											i=a("<div></div>").addClass("stepContainer");
											j=a("<a>"+c.labelNext+"</a>").attr("href","#").addClass("buttonNext");
											r=a("<a>"+c.labelPrevious+"</a>").attr("href","#").addClass("buttonPrevious");
											q=a("<a>"+c.labelFinish+"</a>").attr("href","#").addClass("buttonFinish");
											if(c.errorSteps&&c.errorSteps.length>0){
												a.each(c.errorSteps,function(B,C){e(C,true)})
												}
												i.append(A);
												w.append(m);
												o.append(i);
												o.append(w);
												if(c.includeFinishButton){
													w.append(q)}
													w.append(j).append(r);
													t=i.width();
													a(j).click(function(){if(a(this).hasClass("buttonDisabled")){
														return false
														}
														g();
														return false
